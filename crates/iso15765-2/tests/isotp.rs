...
use iso15765_2::{can::{CAN_FRAME_MAX_SIZE, CanIsoTpFrame, CONSECUTIVE_FRAME_SIZE, DEFAULT_PADDING, FIRST_FRAME_SIZE_2004}, FlowControlState, IsoTpFrame};
...
fn test_single() -> anyhow::Result<()> {
    ...
    let frame = CanIsoTpFrame::SingleFrame { data: hex::decode("1001")? };
    assert_eq!(frame.encode(Some(0x00)), data.to_vec());
    Ok(())
}
