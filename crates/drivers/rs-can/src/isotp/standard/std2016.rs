...
use iso15765_2::{FrameType, IsoTpError, ISO_TP_MAX_LENGTH_2004, ISO_TP_MAX_LENGTH_2016};
...
pub(crate) fn new_single<T: AsRef<[u8]>>(data: T) -> Result<IsoTpFrame, IsoTpError> {
...
pub(crate) fn from_data(data: &[u8]) -> Result<Vec<IsoTpFrame>, IsoTpError> {
    let length = data.len();
    match length {
        0 => Err(IsoTpError::EmptyPdu),
