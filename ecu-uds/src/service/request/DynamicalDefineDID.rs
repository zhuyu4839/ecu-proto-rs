...
use crate::utils;
use crate::service::{DynamicallyDID, DefinitionType, DynamicallyMemAddr, RequestData, Configuration};
                ...
                match v {
                    ...
                    DefinitionType::ClearDynamicallyDefinedDataIdentifier => {
                        let dyn_did = match data_len - offset {
                            ...
                            v => Err(InvalidDataLength { expect: 2, actual: v }),
                        }?;
    ...
    fn into(self) -> Vec<u8> {
        ...
        match self {
            ...
            Self::DefineByMemoryAddress {
                ...
                let mut mem_addr = utils::u128_to_vec(memory.0, mem_addr_len, true);
                let mut mem_size = utils::u128_to_vec(memory.1, mem_size_len, true);
                result.append(&mut mem_addr);
                result.append(&mut mem_size);

                others.into_iter()
                    .for_each(|v| {
                        let mut mem_addr = utils::u128_to_vec(v.0, mem_addr_len, true);
                        let mut mem_size = utils::u128_to_vec(v.1, mem_size_len, true);
                        result.append(&mut mem_addr);
                        result.append(&mut mem_size);
                    });
            },
...
mod tests {
    use hex_literal::hex;
    use crate::service::{DynamicallyDID, DynamicallyMemAddr};
