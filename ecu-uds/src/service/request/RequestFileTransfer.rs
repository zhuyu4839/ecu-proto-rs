...
impl RequestData for RequestFileTransferData {
            ...
            Some(v) => {
                ...
                match v {
                    ModeOfOperation::AddFile => {
                        ...
                        offset += 1;
                        let uncompressed_size = utils::slice_to_u128(&data[offset..offset + filesize_len as usize], true);
                        offset += filesize_len as usize;
                        let compressed_size = utils::slice_to_u128(&data[offset..offset + filesize_len as usize], true);
    ...
    fn into(self) -> Vec<u8> {
        ...
        match self {
            ...
            Self::ResumeFile {
                ...
                result.append(&mut utils::u128_to_vec(uncompressed_size, filesize_len as usize, true));
                result.append(&mut utils::u128_to_vec(compressed_size, filesize_len as usize, true));
            },
...
mod tests {
    use hex_literal::hex;
    use crate::service::{Configuration, DataFormatIdentifier, ModeOfOperation, RequestData};
    use super::RequestFileTransferData;
    ...
    fn add_file() -> anyhow::Result<()> {
        ...
        let request = RequestFileTransferData::try_parse(&source[2..], Some(ModeOfOperation::AddFile), &cfg)?;
        match request {
            RequestFileTransferData::AddFile {
                filepath,
                dfi,
                filesize_len,
                uncompressed_size,
