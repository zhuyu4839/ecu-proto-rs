...
impl ResponseData for SecuredTransData {
...
mod tests {
    ...
    use super::{SecuredTransData, SecuredDataTransNegative, SecuredDataTransPositive};
    ...
    fn new() -> anyhow::Result<()> {
        ...
        let response = SecuredTransData::Successful(
                SecuredDataTransPositive::new(
                    apar,
        ...
        let response = SecuredTransData::try_from(&source[1..])?;
        match response {
            SecuredTransData::Successful(v) => {
            ...
            SecuredTransData::Unsuccessful(_) => panic!(),
        }

        let source = hex!("C4002000000601367F2E13FEC9A180ECFF").as_slice();
        let mut apar = AdministrativeParameter::new();
        apar.signed_set(true);
        let response = SecuredTransData::Unsuccessful(
            SecuredDataTransNegative::new(
                apar,
                SignatureEncryptionCalculation::VehicleManufacturerSpecific(0x00),
                0x0136,
        ...
        let response = SecuredTransData::try_from(&source[1..])?;
        match response {
            SecuredTransData::Successful(_) => panic!(),
            SecuredTransData::Unsuccessful(v) => {
                assert_eq!(v.apar.is_signed(), true);
                assert_eq!(v.signature, SignatureEncryptionCalculation::VehicleManufacturerSpecific(0x00));
