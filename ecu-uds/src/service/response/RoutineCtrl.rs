...
impl<'a> TryFrom<&'a [u8]> for RoutineCtrlData {
    type Error = Error;
...
impl Into<Vec<u8>> for RoutineCtrlData {
...
impl ResponseData for RoutineCtrlData {
...
mod tests {
    ...
    use super::RoutineCtrlData;

    #[test]
    fn new() -> anyhow::Result<()> {
        let source = hex!("7101FF01").as_slice();
        let response = RoutineCtrlData::new(
        ...
        let response = RoutineCtrlData::try_from(&source[2..])?;
