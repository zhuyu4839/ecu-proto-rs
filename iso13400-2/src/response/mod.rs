...
use crate::{{ActiveCode, DiagnosticNegativeCode, Entity, Error, FurtherAction, HeaderNegativeCode, LogicAddress, PowerMode}, constant::*, SyncStatus, DiagnosticPositiveCode};
...
impl TryFrom<&[u8]> for HeaderNegative {
    ...
    fn try_from(data: &[u8]) -> Result<Self, Self::Error> {
        ...
        let expected = SIZE_OF_LENGTH + LENGTH_OF_HEADER_NEGATIVE;
        ...
        if length != LENGTH_OF_HEADER_NEGATIVE {
            return Err(Error::InvalidDataLen { actual: length, expected: LENGTH_OF_HEADER_NEGATIVE });
        }
...
impl Into<Vec<u8>> for HeaderNegative {
    fn into(self) -> Vec<u8> {
        ...
        let length = LENGTH_OF_HEADER_NEGATIVE as u32;
...
impl TryFrom<&[u8]> for VehicleID {
    ...
    fn try_from(data: &[u8]) -> Result<Self, Self::Error> {
        let data_len = data.len();
        let expected = SIZE_OF_LENGTH + MIN_VEHICLE_ID_LENGTH;
        if data_len < expected {
            return Err(Error::InvalidLength { actual: data_len, expected });
        }

        let length = u32::from_be_bytes(data[..SIZE_OF_LENGTH].try_into().unwrap()) as usize;
        if length < MIN_VEHICLE_ID_LENGTH {
            return Err(Error::InvalidDataLen { actual: length, expected: MIN_VEHICLE_ID_LENGTH });
        }
        ...
        let sync_status = match data_len - offset {
            ...
            _ => Err(Error::InvalidLength { actual: data_len, expected: LENGTH_OF_VIN + 1 })
        }?;
...
impl Into<Vec<u8>> for VehicleID {
    fn into(self) -> Vec<u8> {
        ...
        let mut length = MIN_VEHICLE_ID_LENGTH as u32;
...
impl TryFrom<&[u8]> for EntityStatus {
    ...
    fn try_from(data: &[u8]) -> Result<Self, Self::Error> {
        let data_len = data.len();
        let expected = SIZE_OF_LENGTH + MIN_OF_ENTITY_STATUS;
        if data_len < expected {
            return Err(Error::InvalidLength { actual: data_len, expected });
        }

        let length = u32::from_be_bytes(data[..SIZE_OF_LENGTH].try_into().unwrap()) as usize;
        if length < MIN_OF_ENTITY_STATUS {
            return Err(Error::InvalidDataLen { actual: length, expected: MIN_OF_ENTITY_STATUS });
        }
        ...
        let max_data_size = match data_len - offset {
            ...
            _ => Err(Error::InvalidLength { actual: data_len, expected: MIN_OF_ENTITY_STATUS+4 }),
        }?;
...
impl Into<Vec<u8>> for EntityStatus {
    fn into(self) -> Vec<u8> {
        ...
        let mut length = MIN_OF_ENTITY_STATUS as u32;
...
const LENGTH_OF_ALIVE_CHECK: usize = ADDRESS_SIZE;
impl TryFrom<&[u8]> for AliveCheck {
    type Error = Error;
    fn try_from(data: &[u8]) -> Result<Self, Self::Error> {
        let data_len = data.len();
        let expected = SIZE_OF_LENGTH + LENGTH_OF_ALIVE_CHECK;
        if data_len != expected {
            return Err(Error::InvalidLength { actual: data_len, expected });
        }

        let length = u32::from_be_bytes(data[..SIZE_OF_LENGTH].try_into().unwrap()) as usize;
        if length != LENGTH_OF_ALIVE_CHECK {
            return Err(Error::InvalidDataLen { actual: length, expected: LENGTH_OF_ALIVE_CHECK });
        }
...
impl Into<Vec<u8>> for AliveCheck {
    fn into(self) -> Vec<u8> {
        ...
        let length = LENGTH_OF_ALIVE_CHECK as u32;
        result.extend(length.to_be_bytes());
        let src_addr: u16 = self.src_addr.into();
        result.extend(src_addr.to_be_bytes());
        result
...
impl TryFrom<&[u8]> for DiagnosticPositive {
    ...
    fn try_from(data: &[u8]) -> Result<Self, Self::Error> {
        let data_len = data.len();
        let expected = SIZE_OF_LENGTH + MIN_DIAG_POSITIVE_LENGTH;
        if data_len < expected {
            return Err(Error::InvalidLength { actual: data_len, expected });
        }
        let length = u32::from_be_bytes(data[..SIZE_OF_LENGTH].try_into().unwrap()) as usize;
        if length < MIN_DIAG_POSITIVE_LENGTH {
            return Err(Error::InvalidDataLen { actual: length, expected: MIN_DIAG_POSITIVE_LENGTH });
        }
...
impl Into<Vec<u8>> for DiagnosticPositive {
    fn into(mut self) -> Vec<u8> {
        ...
        let length = (MIN_DIAG_POSITIVE_LENGTH + self.pre_diag_msg.len()) as u32;
        result.extend(length.to_be_bytes());
        let src_addr: u16 = self.src_addr.into();
        result.extend(src_addr.to_be_bytes());
        let dst_addr: u16 = self.dst_addr.into();
...
impl TryFrom<&[u8]> for DiagnosticNegative {
    ...
    fn try_from(data: &[u8]) -> Result<Self, Self::Error> {
        let data_len = data.len();
        let expected = SIZE_OF_LENGTH + MIN_DIAG_NEGATIVE_LENGTH;
        if data_len < expected {
            return Err(Error::InvalidLength { actual: data_len, expected });
        }

        let length = u32::from_be_bytes(data[..SIZE_OF_LENGTH].try_into().unwrap()) as usize;
        if length < MIN_DIAG_NEGATIVE_LENGTH {
            return Err(Error::InvalidDataLen { actual: length, expected: MIN_DIAG_NEGATIVE_LENGTH });
        }
...
impl Into<Vec<u8>> for DiagnosticNegative {
    fn into(mut self) -> Vec<u8> {
        ...
        let length = (MIN_DIAG_NEGATIVE_LENGTH + self.pre_diag_msg.len()) as u32;
        result.extend(length.to_be_bytes());
        let src_addr: u16 = self.src_addr.into();
        result.extend(src_addr.to_be_bytes());
        let dst_addr: u16 = self.dst_addr.into();
