...
impl From<u8> for Version {
    fn from(version: u8) -> Self {
        match version {
            ...
            0xFF => Self::Default,
            _ => {
                log::warn!("DoIP - used reserved version: {}", version);
...
impl From<u8> for HeaderNegativeCode {
    fn from(code: u8) -> Self {
        match code {
            ...
            _ => {
                log::warn!("DoIP - used reserved header negative code: {}", code);
                Self::Reserved(code)
            },
        }
    }
...
impl From<u16> for LogicAddress {
    fn from(value: u16) -> Self {
        match value {
            ...
            _ => {
                log::warn!("DoIP - used reserved logic address: {}", value);
                Self::Reserved(value)
            },
        }
    }
...
impl From<u8> for Entity {
    fn from(value: u8) -> Self {
        match value {
            ...
            0x01 => Self::Node,
            _ => {
                log::warn!("DoIP - used reserved entity: {}", value);
                Self::Reserved(value)
            },
        }
    }
...
impl From<u8> for FurtherAction {
    fn from(value: u8) -> Self {
        match value {
            0x00 => Self::NoAction,
            0x01..=0x0F => {
                log::warn!("DoIP - used reserved further action: {}", value);
                Self::Reserved(value)
            },
...
impl From<u8> for SyncStatus {
    fn from(value: u8) -> Self {
        match value {
            ...
            _ => {
                log::warn!("DoIP - used reserved VIN/GID sync. status: {}", value);
                Self::Reserved(value)
            },
        }
    }
...
impl From<u8> for ActiveCode {
    fn from(v: u8) -> Self {
        match v {
            ...
            _ => {
                log::warn!("DoIP - used reserved active code: {}", v);
                Self::Reserved(v)
            },
        }
    }
...
impl From<u8> for PowerMode {
    fn from(value: u8) -> Self {
        match value {
            ...
            _ => {
                log::warn!("DoIP - used reserved power mode: {}", value);
                Self::Reserved(value)
            },
        }
    }
...
impl From<u8> for RoutingActiveType {
    fn from(value: u8) -> Self {
        match value {
            ...
            0x01 => Self::WWHODB,
            0x02..=0xDF => {
                log::warn!("DoIP - used reserved routing active type: {}", value);
                Self::Reserved(value)
            },
...
impl From<u8> for DiagnosticPositiveCode {
    fn from(value: u8) -> Self {
        match value {
            0x00 => Self::Confirm,
            _ => {
                log::warn!("DoIP - used reserved diagnostic positive code: {}", value);
                Self::Reserved(value)
            },
        }
    }
...
impl From<u8> for DiagnosticNegativeCode {
    fn from(value: u8) -> Self {
        match value {
            ...
            _ => {
                log::warn!("DoIP - used reserved diagnostic negative code: {}", value);
                Self::Reserved(value)
            },
        }
    }
